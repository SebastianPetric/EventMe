...
import thesis.hfu.eventme.adapter.*;
import thesis.hfu.eventme.functions.BuildJSON;
import thesis.hfu.eventme.functions.CheckSharedPreferences;
import thesis.hfu.eventme.functions.StartActivity;
import thesis.hfu.eventme.objects.*;
...
public class DBfunctions {
    ...
    public void createTask(final Context context, final int event_id, String admin_id, int editor_id, String task, String description, String quantity) {
        ...
        DBconnection.post(URL_CREATE_TASK, params, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    Toast.makeText(context.getApplicationContext(), response.getString(MESSAGE), Toast.LENGTH_SHORT).show();
                    StartActivity.getInstance().startAllTasksActivity(context.getApplicationContext(), event_id);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
            }
        ...
        DBconnection.post(URL_DELETE_TASK,params,new JsonHttpResponseHandler(){
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    Toast.makeText(context.getApplicationContext(), response.getString(MESSAGE), Toast.LENGTH_SHORT).show();
                    if(response.getInt(STATUS)==200){
                        StartActivity.getInstance().startAllTasksActivity(context, event_id);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
        ...
        DBconnection.post(URL_DELETE_EVENT,params,new JsonHttpResponseHandler(){
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    Toast.makeText(context.getApplicationContext(), response.getString(MESSAGE), Toast.LENGTH_SHORT).show();
                    if(response.getInt(STATUS)==200){
                        StartActivity.getInstance().startAllEventsActivity(context);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    public void addFriendToTask(Activity context, ArrayList<User> userList,int position, int event_id){
        ...
        StartActivity.getInstance().startCreateTaskFromFriendsActivity(context.getApplicationContext(), event_id);
        context.finish();
    }
